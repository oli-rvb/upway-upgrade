...
| `index.html` | Side-by-side comparison page (repo root). Loads `upway.html` and `site/ebikeshop.html` in two iframes with a draggable divider; toolbar lets visitors switch between original-only, split (vertical or horizontal), and upgrade-only. |
